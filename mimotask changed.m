...
for i = 1:length(EbN0)  %%Loop all EbN0  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    ...
    for k=1:L
        ...
        for m=Nt-1:-1:1 %loop all layers
            % changed
                y(:,m)=y(:,m)-r(m+1:Nt,m).*y(:,m+1:Nt);  
                y2(:,m)=y2(:,m)-r(m+1:Nt,m).*s((k-1)*N+1:k*N,m+1:Nt);   % ZF with perfect channel knowledge
...
for i = 1:length(EbN0)  %%Loop all EbN0  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for k=1:L
        ...
        for m=Nt-1:-1:1
            w=h(1:Nt-1,:)'*inv(h(1:Nt-1,:)*h(1:Nt-1,:)'+2*sigma1.^2*diag(ones(1,m))); % w matrix update
            tempy2=tempy*w;
